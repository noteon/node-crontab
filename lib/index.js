...
function CronTab(u, cb) {
  ...
  this.jobs = function(options) {
    ...
    for (var i = 0; i < jobs.length; i++) {
      ...
      for (var j = 0; j < queries.length; j++) {
        ...
        //console.log(job[query](), "match", options[query])
        var jobQuery=job[query]();
        var optQuery=options[query];
        if (_.isRegExp(optQuery)){
          if (!jobQuery.match(optQuery)) {
            match = false;
            break;
          }
        }else if (jobQuery!==optQuery){
          match=false;
          break;
        }
      }
  ...
  this.create = function(command, when, comment) {
    ...
    if (job) {
      jobs.push(job);
      lines.push(job); //change by qinghai
    }
